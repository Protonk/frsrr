...
- PackageQAAgent → Exercise `testthat` to keep the R API, documentation, and the C++ bridge coherent.
...
- PackageQAAgent:
  - Tests: run the VS Code task "R: Test" located in `.vscode/tasks.json`.
...
- Release validation: `R CMD build .` followed by `R CMD check frsrr_<version>.tar.gz` confirms documentation, compiled code, and tests, mirroring CRAN expectations; inspect `frsrr.Rcheck/tests/*.Rout` for failures. Run only when requested by the user.
...
- status of `devtools` in the Codex environment. 
