== Catalog ==
This package is an R/C++ lab for diagnosing Fast Reciprocal Square Root (FRSR) behavior; agents map to the main workflows exposed in `R/`, `src/`, and `tests/testthat/`.
...
- SamplerAgent: `frsrr::frsr_sample()` (`R/sample.R`) draws floats via `.Call('_frsrr_bounded_stratified_sample', ...)` (`src/search.cpp`) before piping through `frsr(detail = TRUE, ...)`.
...
- SamplerAgent: inputs `n`, float/magic bounds (`x_min`, `x_max`, `magic_min`, `magic_max`), `weighted`, and Newton args via `...`; outputs the same diagnostic schema as `frsr(detail = TRUE)` with reproducible samples.
...
== Runtime config ==
- Reproducibility: README’s guarantees come from `Rcpp::RNGScope` in every `.Call` (`src/frsr.cpp`, `src/search.cpp`, `src/phase.cpp`), so `set.seed()` in R governs all randomness; document the seed when sharing diagnostics.
...
- C++ sources (`src/frsr.cpp`, `src/search.cpp`, `src/phase.cpp`, plus `frsr.h`, `search.h`, `Makevars`) showed the parallel/RNG behavior backing each tool.
- tests/testthat/ supplied behavioral expectations covering every exported helper and the underlying samplers.

== Unknowns ==
- Whether `pkgload`, `devtools`, and any compiler toolchains (clang++, g++) are installed on the target runtime.
...
- Expectations for documenting custom Newton formulas beyond the examples in README/man files.
